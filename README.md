...
daisy. WiFi networks are managed from a companion Android app — no reflashing to
change networks.
...
- `bblanchon/ArduinoJson` — Nightscout JSON + network-list storage
...
- **Nightscout is fetched over verified HTTPS.** The device checks the site's
  ...
  **no insecure fallback** — a failed handshake shows an error rather than
...
- Secrets live only in `include/config.h` (gitignored). WiFi passwords and the
  Nightscout token are compiled into the firmware image in plaintext — physical
  possession of the board exposes them, so treat a lost device accordingly
